...
template<class T>
void SparseMatrix<T>::operator=(SparseMatrix<T>&& M)
{
    _Rows = std::move(M._Rows);
    _RowCount = M._RowCount;
    _ColCount = M._ColCount;
...
template<class T>
void SparseMatrix<T>::operator *= (T Scale)
{
    Multiply(*this, Scale);
}

template<class T>
void SparseMatrix<T>::operator += (const SparseMatrix &M)
{
    Assert(M.RowCount() == _RowCount &&
             M.ColCount() == _ColCount, "Invalid matrix addition dimensions.");
    
    for(UINT i = 0; i < _RowCount; i++)
    {
        for(UINT i2 = 0; i2 < M.Rows()[i].Data.Length(); i2++)
        {
            const SparseElement<T> *Element = &M.Rows()[i].Data[i2];
            PushElement(i, Element->Col, Element->Entry);
        }
    }
}
