...
namespace Engine
{
    public class ColorTemplate
    {
        ...
        //writing and loading templates from a file
        /*public void WriteToFile(String filename)
        {
            List<String> lines = new List<String>();
            
            //write the width and height of the image, the number of colors in the palette, and the number of segments
            lines.Add(template.GetLength(0) + " " + template.GetLength(1) + " " + originalPalette.colors.Count() + " " + segToColor.Count());

            int width = template.GetLength(0);
            int height = template.GetLength(1);

            //write out the template colors, one per line
            for (int j = 0; j < height; j++)
                for (int i = 0; i < width; i++)
                    lines.Add(template[i, j].L + " " + template[i, j].A + " " + template[i, j].B);


            //write out the original palette, one per line (in rgb)
            foreach (Color color in originalPalette.colors)
                lines.Add(color.R + " " + color.G + " " + color.B);
            
            //write out all the slot information
            //write out the slot assignments, one per line
            for (int j = 0; j < height; j++)
                for (int i = 0; i < width; i++)
                    lines.Add(slots.assignments[i, j].ToString());

            //write out slot counts
            for (int i = 0; i < slots.counts.Length; i++)
                lines.Add(slots.counts[i].ToString());

            //write out slot to mean color in LAB space
            for (int i = 0; i < slots.segToMeanColor.Length; i++)
                lines.Add(slots.segToMeanColor[i].L + " " + slots.segToMeanColor[i].A + " " + slots.segToMeanColor[i].B);
           
            
            //write out the slots id to color index, one per line
            for (int i = 0; i < segToColor.Count(); i++)
                lines.Add(segToColor[i].ToString());
        ...
        //load from file
        public ColorTemplate(String filename)
        {
            //read in all the lines
            String[] lines = File.ReadAllLines(filename);

            //read in the width, height, number of colors, and number of slots
        ...
        public ColorTemplate(Bitmap image, PaletteData palette)
        {
            ...
            //CIELAB[,] recolored = SoftModeRecolor(Util.Map<Color, CIELAB>(Util.BitmapToArray(image), c => Util.RGBtoLAB(c)), palette);
            ...
            for (int i = 0; i < slots.numSegments; i++)
                segToColor[i] = i;

            originalPalette = new PaletteData(palette);
        }

        //Note: this method treats color groups as defined in the original segmentation, no matter
        //if two are mapped to the same palette color. Currently not using this method.
       ...
       /* private bool inBounds(int x, int y, int width, int height)
        ...
        private CIELAB[,] SoftModeRecolor(CIELAB[,] image, PaletteData palette)
        {
            int width = image.GetLength(0);
            int height = image.GetLength(1);
            CIELAB[,] result = new CIELAB[width, height];
            ...
            for (int i = 0; i < width; i++)
            {
                for (int j = 0; j < height; j++)
                {
                    ...
                    //otherwise, find the best color by soft mode
                    for (int dx = -1; dx <= 1; dx++)
                    {
                        for (int dy = -1; dy <= 1; dy++)
                        {
                            ...
                            if (Util.InBounds(x, y, width, height))
                            {
                                for (int c = 0; c < palette.colors.Count(); c++)
                                {
                                    double cscore = Math.Sqrt(image[x, y].SqDist(palette.lab[c]));//int bestc = ClosestColorIndex(image[x, y], palette.lab);
                                    scores[c] += weight*cscore;
                                }
                            }
                        }
                    }
        ...
        private CIELAB[,] ModeRecolor(CIELAB[,] image, PaletteData palette)
        {
            ...
            for (int i = 0; i < width; i++)
            {
                for (int j = 0; j < height; j++)
                {
                    ...
                    {
                        //if the color is transparent, set it to a transparent color and continue
                        result[i, j] = color;
                        continue;
                    }
        ...
        public Bitmap SolidColor(PaletteData palette)
        {
            Bitmap result = SolidColor(palette, segToColor);
            return result;
        }
