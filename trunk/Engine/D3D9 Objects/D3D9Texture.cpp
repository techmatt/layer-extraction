...
{
    FreeMemory();
    HRESULT hr = D3DXCreateTextureFromFileEx(_Device, filename.CString(), 0, 0, 0, 0, format, D3DPOOL_MANAGED, D3DX_DEFAULT, D3DX_DEFAULT, 0, NULL, NULL, &_Texture);
    if(FAILED(hr) && hr != 0x8876086c)
    {
#ifdef _DEBUG
        Utility::MessageBox("Failed to load texture file");
#endif
        _Texture = NULL;
    }
    else
    {
        ...
        //
        // GetSurfaceLevel will fail for compressed textures
        //
        _Texture->GetSurfaceLevel(0, &_SurfaceTopLevel);
    }
}
