...
public:
    ...
    void operator=(SparseMatrix<T>&& V);
    ...
    //
    void operator *= (T B);
    void operator += (const SparseMatrix &s);
    ...
    __forceinline const SparseRow<T>& operator[](UINT k) const
    {
        return _Rows[k];
    }
    __forceinline SparseRow<T>& operator[](UINT k)
    {
        return _Rows[k];
    }
