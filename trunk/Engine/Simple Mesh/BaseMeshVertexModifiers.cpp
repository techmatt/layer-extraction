...
void BaseMesh::ColorNormalsGrayScale(RGBColor Color)
{
    ...
    {
        Vec3f normal = V[i].Normal;
        int r = Utility::Bound(int(fabsf(normal.x/2.0f+0.5f)*255), 0, 255);
        int g = Utility::Bound(int(fabsf(normal.y/2.0f+0.5f)*255), 0, 255);
        int b = Utility::Bound(int(fabsf(normal.z/2.0f+0.5f)*255), 0, 255);    //remap each normal's (x, y, z) to (r, g, b)
