...
//
// Stemming algorithm modified from http://tartarus.org/~martin/PorterStemmer/c_thread_safe.txt
//
...
void Stemmer::Stem(String &s)
{
    const int k = int(s.Length()) - 1;
    if (k <= 1)
    {
        return;
    }
    ...
    step4(&z);
    step5(&z);
    while(z.k < int(s.Length()) - 1)
    {
        s.PopEnd();
    }
}

int Stemmer::cons(StemmerInfo *z, int i)
{  switch (z->b[i])
...
int Stemmer::vowelinstem(StemmerInfo *z)
{
   int j = z->j;
...
int Stemmer::doublec(StemmerInfo *z, int j)
{
   char * b = z->b;
   if (j < 1) return FALSE;
...
int Stemmer::cvc(StemmerInfo *z, int i)
...
int Stemmer::ends(StemmerInfo *z, char * s)
{  int length = s[0];
   char * b = z->b;
   int k = z->k;
...
void Stemmer::setto(StemmerInfo *z, char * s)
{  int length = s[0];
   int j = z->j;
...
void Stemmer::r(StemmerInfo *z, char * s) { if (m(z) > 0) setto(z, s); }
...
void Stemmer::step1ab(StemmerInfo *z)
{
   char * b = z->b;
   if (b[z->k] == 's')
...
void Stemmer::step1c(StemmerInfo *z)
{
...
void Stemmer::step2(StemmerInfo *z) { switch (z->b[z->k-1])
{
...
void Stemmer::step3(StemmerInfo *z) { switch (z->b[z->k])
{
...
void Stemmer::step4(StemmerInfo *z)
{  switch (z->b[z->k-1])
...
void Stemmer::step5(StemmerInfo *z)
{
   char * b = z->b;
   z->j = z->k;
   if (b[z->k] == 'e')
